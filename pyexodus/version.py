...
#
# Modified by the ObsPy project and pyexodus.
...
# NO IMPORTS FROM PYEXODUS OR FUTURE! (file gets used at
# installation time)
...
PYEXODUS_ROOT = os.path.abspath(os.path.join(script_dir, os.pardir))
...
def call_git_describe(abbrev=4):  # pragma: no cover
    try:
        p = Popen(['git', 'rev-parse', '--show-toplevel'],
                  cwd=PYEXODUS_ROOT, stdout=PIPE, stderr=PIPE)
        p.stderr.close()
        path = p.stdout.readline().decode().strip()
        p.stdout.close()
    except Exception:
        return None
    if os.path.normpath(path) != PYEXODUS_ROOT:
        return None
    try:
